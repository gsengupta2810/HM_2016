...
//chip,sensor,i2c read/write
int  vmuvc_read_chip(struct uvc_video_device *video, __u16 a,  __u16* v)
{
   	int		result = 0;
	__u8	Buffer[ 4 ];
	__u16	wLength;
	...
	Buffer[ 0 ] = 0x00;
	Buffer[ 1 ] = (__u8)( a & 0x00FF);
	Buffer[ 2 ] = (__u8)( a >> 8 );
	Buffer[ 3 ] = 0;
	wLength = sizeof( Buffer );

	result = uvc_query_ctrl(video->dev, USBVC_SET_CUR, bExtUnitId, video->dev->intfnum, EU_REG_CONTROL, Buffer, wLength);
   	if(result >= 0)
	{
		Buffer[ 0 ] = 0x00;
		Buffer[ 1 ] = (__u8)( a & 0x00FF);
		...
		Buffer[ 3 ] = 0;
		wLength = sizeof( Buffer );
		result = uvc_query_ctrl(video->dev, USBVC_GET_CUR, bExtUnitId, video->dev->intfnum, EU_REG_CONTROL, Buffer, wLength);
		if (result >= 0)
		{
			*v = Buffer[ 3 ];
			uvc_trace(UVC_TRACE_CALLS, "[wlq-uVc] vmuvc_read_chip 0x%08x \n", Buffer[ 3 ] );
		}
		else
		{
			*v = 0x0000;
			uvc_trace(UVC_TRACE_CALLS, "[wlq-uVc] vmuvc_read_chip failed.\n");
		}
	}
	else
	{
...
int  vmuvc_write_chip(struct uvc_video_device *video, __u16 a , __u16 v)
{
   	int		result = 0;
	__u8	Buffer[ 4 ];
	__u16	wLength;
	...
	result = uvc_query_ctrl(video->dev, USBVC_SET_CUR, bExtUnitId, video->dev->intfnum, EU_REG_CONTROL, Buffer, wLength);
   	if(result < 0)
	{
		uvc_trace(UVC_TRACE_CALLS, "[wlq-uVc] vmuvc_read_chip\n");
	}
    return result;
}

int  vmuvc_read_sensor(struct uvc_video_device *video, __u16 a, __u16* v)
{
	int	result = 0;
	__u8	Buffer[ 4 ];
	__u16	wLength;
	...
	//address is byte, value is word
	Buffer[ 0 ] = 0x11;
	Buffer[ 1 ] = (__u8)( a & 0xFF );
	Buffer[ 2 ] = (__u8)( a >> 8 );
	Buffer[ 3 ] = 0;
	wLength = sizeof( Buffer );
   	...
   	if(result >= 0)
	{
		...
		if ( result >= 0 )
		{
			*v = (Buffer[ 3 ] << 8) | Buffer[2];
                        uvc_trace(UVC_TRACE_CALLS, "[wlq-uVc] vmuvc_read_sensor ( 0x%08x )\n", (Buffer[3]<<8 |Buffer[2])   );
		}
		else
		{
			*v = 0x0000;
			uvc_trace(UVC_TRACE_CALLS, "[wlq-uVc] vmuvc_read_sensor\n");
		}
	}
	else
	{
...
int  vmuvc_write_sensor(struct uvc_video_device *video, __u16 a , __u16 v)
{
	int	result = 0;
	__u8	Buffer[ 4 ];
	__u16	wLength;
	...
	if(result < 0)
	{
		uvc_trace(UVC_TRACE_CALLS, "[wlq-uVc] vmuvc_write_sensor\n");
	}
    return result;
}

int  vmuvc_read_i2c(struct uvc_video_device *video, __u8 ia,  __u16 a,  __u16* v)
{
	int	result = 0;
	__u8	Buffer[ 4 ];
	__u16	wLength;
	...
	result = uvc_query_ctrl(video->dev, USBVC_SET_CUR, bExtUnitId, video->dev->intfnum, EU_IIC_DEVICE_REGISTER, Buffer, wLength);
	if(result >= 0)
	{
		result = uvc_query_ctrl(video->dev, USBVC_GET_CUR, bExtUnitId, video->dev->intfnum, EU_IIC_DEVICE_REGISTER, Buffer, wLength);
		if ( result < 0 )
		{
			*v = 0x0000;
			uvc_trace(UVC_TRACE_CALLS, "[wlq-uVc] vmuvc_read_i2c\n");
		}
		else
		{
			*v = Buffer[ 3 ];
uvc_trace( UVC_TRACE_CALLS, "[wlq-uVc] read i2c %02x (%02x, %02x)\n", Buffer[ 3 ] , ia, a);
		}
	}
	else
	{
...
int  vmuvc_write_i2c(struct uvc_video_device *video, __u8 ia,  __u16 a, __u16 v)
{
	int	result = 0;
	__u8	Buffer[ 4 ];
	__u16	wLength;
	...
	if(result < 0)
	{
		uvc_trace(UVC_TRACE_CALLS, "[wlq-uVc] vmuvc_write_i2c\n");
	}
    return result;
}
		     ...
		     unsigned int cmd, void *arg)
{
...
#if 1 //[wlq]
		if (video->dev->udev->descriptor.idProduct) //[wlq]
	...
	case VIDIOC_EU_CTRL: // [wlq]
	{
		struct vm_ext_unit* eu = (struct vm_ext_unit *) arg;
		__u16  tValue = 0;

		//uvc_trace(UVC_TRACE_CONTROL, "in ext unit control case.\n" );	
	
		if( eu->opType == CMD_R )
		{
			if( eu->cmdType == CMD_CC )
			{
				vmuvc_read_chip( video, eu->address, &tValue );
				eu->value = tValue;
			}
			else if( eu->cmdType == CMD_SS )
			{
				vmuvc_read_sensor( video, eu->address, &tValue );
				eu->value = tValue;
			}
			else if( eu->cmdType == CMD_II )
			{
				vmuvc_read_i2c( video, eu->iiaddr, eu->address, &tValue );
				eu->value = tValue;
			}
			else
			{
				uvc_trace( UVC_TRACE_CONTROL, " read command, unknown operation type. \n" );
			}
		}
		else if( eu->opType == CMD_W )
		{
			if( eu->cmdType == CMD_CC )
			{
				vmuvc_write_chip( video, eu->address, eu->value );
			}
			else if( eu->cmdType == CMD_SS )
			{
				vmuvc_write_sensor( video, eu->address, eu->value );
			}
			else if( eu->cmdType == CMD_II )
			{
				vmuvc_write_i2c( video, eu->iiaddr, eu->address, eu->value );
			}
			else
			{
				uvc_trace( UVC_TRACE_CONTROL, " write command, unkown operation type. \n" );
			}
		}
		else
		{
