...
__u8	bProcessingUnit;
__u8	bExtensionUnit;
__u8	bInputTerminal;
__u8	bControlSelectorUnit;
__u8	bExtUnitId;
	...
	while (buflen > 2) {
		...
		case VC_HEADER:
			...
			for (i = 0; i < n; ++i) {
				...
				streaming->intf = intf; //usb_get_intf(intf);  //[wlq]
				streaming->intfnum =
					intf->cur_altsetting->desc.bInterfaceNumber;

				if (uvc_parse_streaming(dev, streaming) < 0) {
				//	usb_put_intf(intf); // [wlq]
					kfree(streaming);
					continue;
				}
		...
		case VC_INPUT_TERMINAL:
        	...
        	uvc_trace(UVC_TRACE_CALLS, "[wlq-uVc] vmuvc_parse_control, Found Input Terminal.\n");
			bInputTerminal = 1; //[wlq-uVc]
			break;
		...
		case VC_SELECTOR_UNIT:
        	...
        	uvc_trace(UVC_TRACE_CALLS, "[wlq-uVc] vmuvc_parse_control, Found Control Selector Unit.\n");
			bControlSelectorUnit = 1; // [wlq-uVc]
			break;
		...
		case VC_PROCESSING_UNIT:
			...
			list_add_tail(&unit->list, &dev->entities);
        	uvc_trace(UVC_TRACE_CALLS, "[wlq-uVc] vmuvc_parse_control, Found Processing Unit.\n");
			bProcessingUnit = 1; //[wlq-uVc]
			break;
		...
		case VC_EXTENSION_UNIT:
			...
			list_add_tail(&unit->list, &dev->entities);
        	uvc_trace(UVC_TRACE_CALLS, "[wlq-uVc] vmuvc_parse_control, Found Extension Unit.\n");
			bExtensionUnit = 1; //[wlq-uVc]
			break;
